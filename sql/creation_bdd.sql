...
-- ============================================================================
-- 1.3 IMPORT DES DONNÉES DEPUIS LES FICHIERS CSV
-- ============================================================================
-- Note : Ces commandes sont à adapter selon votre configuration MySQL
-- Vous devrez peut-être utiliser l'interface graphique de votre client SQL

-- Import PROPRIETAIRE
LOAD DATA LOCAL INFILE '/mnt/user-data/uploads/proprietaire.csv'
INTO TABLE Proprietaire
...
SET 
    ...
    anneeI = NULLIF(@anneeI, '');

-- Import CLIENT
LOAD DATA LOCAL INFILE '/mnt/user-data/uploads/client.csv'
INTO TABLE Client
FIELDS TERMINATED BY ';'
LINES TERMINATED BY '\n'
IGNORE 1 ROWS
(CodeC, Nom, @Prenom, age, @Permis, Adresse, Ville)
SET 
    Prenom = NULLIF(@Prenom, 'NULL'),
    Permis = NULLIF(@Permis, 'NULL');

-- Import VOITURE
LOAD DATA LOCAL INFILE '/mnt/user-data/uploads/voiture.csv'
INTO TABLE Voiture
FIELDS TERMINATED BY ';'
LINES TERMINATED BY '\n'
IGNORE 1 ROWS
...
SET 
    achatA = NULLIF(@achatA, '');

-- Import LOCATION
LOAD DATA LOCAL INFILE '/mnt/user-data/uploads/location.csv'
INTO TABLE Location
FIELDS TERMINATED BY ';'
LINES TERMINATED BY '\n'
IGNORE 1 ROWS
(CodeC, immat, annee, mois, numloc, km, duree, villed, villea, dated, datef, @note, @avis)
SET 
    note = NULLIF(@note, 'NULL'),
    avis = NULLIF(@avis, 'NULL');
